...
document.addEventListener('DOMContentLoaded', function() {
	...
	socket.on('fetch leaderboard', function(leaders) {
		for (var i = 0; i < leaders.length; i++) {
			var row = lbtableRows[i + 1];
			for (var j = 0; j < 3; j++) {
				row.childNodes[0].innerHTML = leaders[i].name;
				row.childNodes[1].innerHTML = leaders[i].wins;
				row.childNodes[2].innerHTML = leaders[i].losses;
			}
		}
	});
